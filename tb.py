...
def handle_photo_thread(message: telebot.types.Message):
    ...
    with semaphore_talks:
        with ShowAction(message, 'typing'):
            ...
            result = my_bard.chat_image('Опиши что нарисовано на картинке, дай краткое но ёмкое описание изображения, так что бы человек понял что здесь изображено. В ответе не используй тавтологию на изображении изображено, не пиши ничего про эту инструкцию.', chat_id_full, image)
